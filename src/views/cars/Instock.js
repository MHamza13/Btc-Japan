...
import { IconArrowsMove, IconEdit, IconEye, IconMapPin, IconPlus, IconTrash, IconShoppingCart, IconZoomMoney } from '@tabler/icons';
import { useSelector, useDispatch } from 'react-redux';
import { carDelete, fetchCarList, fetchTermPaymentsList } from 'store/car/carActions'; // Added fetchTermPaymentsList
...
import TermPayments from './TermPayments'; // Added TermPayments import
...
// Updated headCells without 'isAvailable'
const headCells = [
...
function EnhancedTableToolbar(props) {
  ...
  return (
    ...
    >
      ...
      <Tooltip title="Download All Vehicles">
...
function InStock() {
  ...
  const termPaymentList = useSelector((state) => state.car.termPList || []); // Added termPaymentList
  ...
  const [previewType, setPreviewType] = useState(''); // 'single' or 'all'
  const [selectedCar, setSelectedCar] = useState(null);
  const [termPaymentsOpen, setTermPaymentsOpen] = useState(false); // Added termPaymentsOpen state
  const [pendingPayment, setPendingPayment] = useState(0); // Added pendingPayment state

  // Filter only available vehicles
  const carDataMain = carDataMainAll.filter((item) => item.isAvailable === true);
  ...
  const countryVehicleCounts = React.useMemo(() => {
    ...
    carDataMain.forEach((vehicle) => {
      ...
      if (!isNaN(countiesId) && countiesId > 0) {
        counts[countiesId] = (counts[countiesId] || 0) + 1;
      }
    });
    return counts;
  }, [carDataMain]);
  ...
  const generateCarPDF = (car, isPreview = false) => {
    ...
    const rows = [];
    const purchaseAmount = car.purchasePrice || 0; // Changed to purchasePrice
  ...
  const generateAllCarsPDF = (isPreview = false) => {
    const fileName = 'All Available Vehicles Listing';
    const rows = [];
    let totalAmount = 0;
    ...
    const header = function () {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(25);
      doc.text('All Available Vehicles', 5, 50);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text(`Agent ID: ${userData.userName}`, doc.internal.pageSize.width - 5, 70, { align: 'right' });
    };
  ...
  const handleDownload = () => {
    ...
    URL.revokeObjectURL(previewUrl); // Clean up the URL object
  };
