...
import { fetchEditCustomerList, fetchLedgerList } from 'store/customer/customerActions';
...
const columns = ['Model', 'Pending Amount', 'You Give'];
...
function CustomerDetails() {
  ...
  useEffect(() => {
    if (editCustomerList && Object.keys(editCustomerList).length > 0) {
      const updatedDetails = {
        customerName: `${editCustomerList.firstName || ''} ${editCustomerList.lastName || ''}`.trim() || 'Unknown Customer',
        contactNumber: editCustomerList.phoneNumber || '',
        email: editCustomerList.email || '',
        advanceAmount: editCustomerList.advanceAmount || 0
      };
      setCustomerDetails(updatedDetails);
    }
  }, [editCustomerList, id]);

  const sortedData = [...ledgerList].sort((a, b) => new Date(b.purchaseDate || 0) - new Date(a.purchaseDate || 0));
  const filteredData = sortedData.filter((item) => item.chassisNo?.toLowerCase().includes(searchTerm.toLowerCase()) || false);

  const totalPendingAmount = filteredData.reduce((sum, i) => sum + (Number(i.pendingAmount) || 0), 0);
  const totalPrice = filteredData.reduce((sum, i) => sum + (Number(i.price) || 0), 0);
  const paymentMade = totalPrice - totalPendingAmount;
  const existingCurrency = filteredData.length > 0 ? filteredData[0].currency || '' : '';
  const latestPurchaseDate = filteredData.length > 0 ? filteredData[0].purchaseDate || '' : '';

  const generatePDF = (isPreview = false) => {
    setPdfLoading(true);
    const doc = new jsPDF();
    const fileName = `${customerDetails.customerName} - Ledger Details`;
    ...
    const header = function () {
      ...
      doc.setFontSize(25);
      doc.text(customerDetails.customerName, 5, 50);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text(`Phone: ${customerDetails.contactNumber || 'N/A'}`, 5, 55);
      doc.text(`Email: ${customerDetails.email || 'N/A'}`, 5, 60);
  ...
  // Navigate to ledger page instead of opening modal
  ...
  const handleWhatsAppClick = () => {
    const phoneNumber = customerDetails.contactNumber.replace(/\D/g, '');
  ...
  const handleSMSClick = () => {
    const phoneNumber = customerDetails.contactNumber;
    if (phoneNumber && phoneNumber !== '') {
  ...
  return (
    <>
      ...
      <StyledBox component={Paper} style={{ padding: 0 }}>
        <HeaderContainer>
          <HeaderImage src={bgImage} alt="Header Background" />
          <CustomerName style={{ marginTop: '75px' }} variant="h2">
          ...
          <PriceBox style={{ zIndex: '9' }}>
            <Typography variant="h5" sx={{ color: '#388e3c', fontWeight: 'bold', fontSize: '28px' }}>
        ...
        <ButtonContainer>
          <StyledButton type="button" onClick={handleNavigateToLedger}>
        ...
        <ContentWrapper style={{ margin: 'auto', boxShadow: 'none' }}>
          ...
          <TableContainer component={Paper} sx={{ borderRadius: '8px', mt: 4 }}>
            <Table sx={{ minWidth: 650 }} aria-label="customer ledger table">
              <TableHead>
                <TableRow>
        ...
        <Modal open={openGetModal} onClose={handleCloseGetModal} aria-labelledby="get-modal-title" aria-describedby="get-modal-description">
          <ModalBox>
                ...
                <TableBody>
                  {filteredData.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell sx={{ color: '#d32f2f', fontWeight: 'bold' }}>
                        {row.pendingAmount ? `${existingCurrency} ${addThousandSeparator(row.pendingAmount)}` : `${existingCurrency} 0`}
                      </TableCell>
                      <TableCell>{row.purchaseDate ? moment(row.purchaseDate).format('YYYY-MM-DD') : 'N/A'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <Typography variant="h6" sx={{ mt: 2, color: '#d32f2f' }}>
              Total You will get: {existingCurrency} {addThousandSeparator(totalPendingAmount)}
            </Typography>
        ...
        >
          <ModalBox>
                    ...
                    <TableRow key={row.id}>
                      ...
                      <TableCell sx={{ color: '#388e3c', fontWeight: 'bold' }}>
                        {paymentMade > 0
                          ? `${existingCurrency} 0`
                          : row.pendingAmount
                          ? `${existingCurrency} ${addThousandSeparator(row.pendingAmount)}`
                          : `${existingCurrency} 0`}
                      </TableCell>
                      <TableCell>{row.purchaseDate ? moment(row.purchaseDate).format('YYYY-MM-DD') : 'N/A'}</TableCell>
                    </TableRow>
            ...
            <Typography variant="h6" sx={{ mt: 2, color: '#388e3c' }}>
              Total You will give: {existingCurrency} {addThousandSeparator(paymentMade > 0 ? 0 : totalPendingAmount)}
            </Typography>
        ...
        >
          <PdfPreviewBox>
            <Typography id="pdf-preview-title" variant="h6" component="h2" sx={{ mb: 2 }}>
              {customerDetails.customerName} Ledger PDF Preview
            </Typography>
            {pdfLoading ? (
