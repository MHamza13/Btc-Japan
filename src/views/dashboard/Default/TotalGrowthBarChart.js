...
const TotalGrowthBarChart = (props, { isLoading }) => {
  ...
  const perChartList = useSelector((state) => state.dashboard.pChartList);
  ...
  const totalPurchasesArray = perChartList.map(item => item.totalPurchases.toString());
  const purchaseMonthArray = perChartList.map(item => {
  const date = new Date(item.purchaseMonth);
...
TotalGrowthBarChart.propTypes = {
  ...
  setPurchaseChartDate: PropTypes
};
