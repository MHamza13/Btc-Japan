...
import { fetchSaleCountryReportList } from 'store/reports/reportsActions';
...
function SaleCountryReport(){
    const dispatch = useDispatch();
    const loading = useSelector((state) => state.reports.loading);
    const sCReportData = useSelector((state) => state.reports.sCReportList);
    ...
    useEffect(()=>{
        dispatch(fetchSaleCountryReportList(selectCountry))
        dispatch(fetchCountryList())
    }, [dispatch, selectCountry])
    
    const filteredData = sCReportData.filter(row =>
    ...
    const handelDownloadPdf = () =>{
        ...
        const rows = filteredData.map(row => [
            ...
            addThousandSeparator(row.profit)+' '+row.saleCurrency,
            addThousandSeparator(row.loss)+' '+row.saleCurrency
        ]);
                ...
                <TableBody>
                    {(rowsPerPage > 0
                        ...
                        <TableRow key={row.id}>
                            ...
                            <TableCell>{addThousandSeparator(row.profit)} {row.saleCurrency}</TableCell>
                            <TableCell>{addThousandSeparator(row.loss)} {row.saleCurrency}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
                </Table>
...
export default SaleCountryReport;